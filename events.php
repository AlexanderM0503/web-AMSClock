    ...
    if (isset($_POST["action"]))
    {
        ...
        if ($_POST["action"] == "add")
        {
            if (($_POST["addDate"] != "") && ($_POST["addTitle"] != "") && ($_POST["addText"] != ""))
            {
                ...
                $stmt = $db->prepare($sql);
                $stmt->bindValue(":date_evnt", $_POST["addDate"], SQLITE3_TEXT);
                $stmt->bindValue(":title_evnt", $_POST["addTitle"], SQLITE3_TEXT);
                $stmt->bindValue(":text_evnt", $_POST["addText"], SQLITE3_TEXT);
                $result = $stmt->execute();
                $stmt->close();
                $db->close();                
            }
        ...
        {
            $db = new SQLite3($CONF["db_file"]);
            $sql = "DELETE FROM events WHERE id_evnt = :id_evnt";
            $stmt = $db->prepare($sql);
            $stmt->bindValue(":id_evnt", $_POST["id_evnt"], SQLITE3_INTEGER);
            $result = $stmt->execute();
            $stmt->close();
            $db->close();
        }
    ...
    <body>
        <?php
            ...
            if ($_SESSION["login"] == true)
            {
                ...
                {
                    print("<div class='event'>");
                    print("    <div class='title'>Нет событий</div>");
